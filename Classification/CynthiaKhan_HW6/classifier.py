import re
from data_processor import dataset,feature_set,no_of_items
...
with open('test_remarks.txt', 'r') as f:
    for row in f:
        result = naive_bayes(row)
...
accuracy = (len([i for i, j in zip(actual, predicted) if i == j])/len(actual))*100
